...
def _move_var_decls_to_top_of_scope(node, vars_already_scoped={}):
    ...
    Returns:
        ...
        vars_already_scoped: hashtable with variables scoped so far in program."""

    current_scope_new_vars = []

    if isinstance(node, ast.FunctionDef) or isinstance(node, ast.Module):
        for child_node in ast.iter_child_nodes(node):
            new_vars, vars_already_scoped = _find_vars_in_same_scope(child_node, vars_already_scoped)
...
def _find_vars_in_same_scope(node, vars_already_scoped):
    ...
    Note: Will exceed maximum stack depth if exceddingly complex AST's are put in.
        Parameters:
            node: ast node to find all child variables
            vars_previously_declared: vars that have already been declared at a higher scope
                so no need to reinitilize
        Returns:
    ...
    #Recur down all children to find more vars, unless the node opens a new scope.
    elif not isinstance(node, ast.Module) and not isinstance(node, ast.FunctionDef):
        for child_node in ast.iter_child_nodes(node):
            new_vars, vars_already_scoped = _find_vars_in_same_scope(child_node, vars_already_scoped)
            current_scope_new_vars = current_scope_new_vars + new_vars
...
def _move_initilizations_to_top_of_scope(node, vars_to_re_init):
    ...
    for var in vars_to_re_init:
        node.body = \
          ...
          ], value=ast.NameConstant(None))] + node.body
